...
/* inline media: embedded reels, visual carousels, static mocks */
.media{margin-top:12px}
.igwrap{max-width:300px;margin:8px auto;background:#000;border-radius:18px;overflow:hidden;border:1px solid var(--border);padding:6px;box-shadow:0 14px 34px -18px #000}
...
.posts{column-count:2;column-gap:18px}
.post{background:var(--navy2);border:1px solid var(--border);border-radius:14px;padding:20px;border-top:4px solid var(--c);break-inside:avoid;-webkit-column-break-inside:avoid;width:100%;display:inline-block;margin:0 0 18px}
@media(max-width:760px){.posts{column-count:1}}
...
${c.reels.map(r=>`<div class="refnote">🎬 <b>${esc(r[0])}</b> — Copy this: ${esc(r[2])}</div>${igFrame(r[1])}`).join('')}</div>`).join('');
/* posts */
/* ---- inline media helpers (embed videos / render carousels & statics in-dashboard, no redirect) ---- */
...
function staticMock(p){
 return `<div class="smock"><div class="sbrand">PANTHEON</div><div class="shead" ${ce()}>${esc(p.hook||p.title)}</div><div class="ssub" ${ce()}>${esc(p.cta)}</div><div class="sfoot">@pantheon_development</div></div>`;}
function mediaBlock(p){
 if(p.ftype==='Reel'){
   return `<div class="media">${p.ref.length?igFrame(p.ref[0][1]):''}${refNote(p)}</div>`;}
 if(p.ftype==='Carousel'){
   return `<div class="media">${carouselDeck(p)}${p.ref.length?`<details><summary>🎬 Style reference (video)</summary><div class="dbody">${igFrame(p.ref[0][1])}${refNote(p)}</div></details>`:''}</div>`;}
...
function briefBlock(p){
 if(p.ftype==='Carousel'){
   return `<details><summary>🗂 Slide copy (${p.detail.length} slides)</summary>
...
function postCard(p){const pl=PILLAR_BY[p.pillar];
 ...
 ${briefBlock(p)}
 <div class="storyline" ${ce()}>📱 Story tie-in: ${esc(p.story)}</div></div>`;}
